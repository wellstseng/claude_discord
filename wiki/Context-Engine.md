...
## Tool LRU Eviction（4-26）

**問題**：`tool_search` 活化（deferred tool 把 schema 撈進 context）造成 tool definition cache 在長 turn 中累積，prompt 體積不斷膨脹，每輪付出加倍 cache cost。

**解法**：CE 觀察視野擴大 + 工具結果 LRU eviction：

- **視野擴大**：CE 從只看「最近 N turn」擴大到「整個 session 的 tool_use / tool_result 對」，能準確估算累積 token
- **Tool LRU eviction**：超出預算時依「最近最少使用」順序汰除舊 tool definition / 舊 tool_result（保留 in-flight 的 pairing），治本 tool_search 重複活化造成的長期膨脹

行為與 message decay 互補：decay 處理「順序性的舊」，LRU 處理「重複性的舊」。
...
| 日期 | 變更 |
|------|------|
| 2026-04-26 | CE 視野擴大 + Tool LRU Eviction（治本 tool_search 活化造成的 cache 累計成本） |
