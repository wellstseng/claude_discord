...
建立 Discord Client，處理 `messageCreate` + `messageUpdate` 事件：bot 自身過濾 → 訊息去重 → `getChannelAccess()` 查詢 per-channel 設定 → allowBot/allowFrom 過濾 → requireMention 判斷 → 附件下載 → debounce 合併 → 觸發 agentLoop + reply-handler。
...
| `debounceImages` | key | `Array<{data, mimeType, name}>`（累積圖片附件） |
| `debounceMessages` | key | 第一則 `Message`（用於 reply，Discord 引用回覆需要此物件） |
| `debounceMessageIndex` | messageId | `{ key, idx }`（訊息在 buffer 中的位置，供 messageUpdate 替換用） |
...
## 訊息編輯處理（messageUpdate）

`handleMessageEdit()` 處理 Discord 訊息編輯事件，分兩階段：

| 階段 | 條件 | 行為 |
|------|------|------|
| A. Debounce 窗口內 | `debounceMessageIndex.has(messageId)` | 直接替換 buffer 中對應位置的文字，agent 拿到修正後版本 |
| B. 已 dispatch | channel 有 active CLI Bridge 且 `status=busy` | 透過 `bridge.send()` 注入 `[訊息編輯]` 通知，中斷舊 turn |
| 無處理 | 其他情況（agent loop 進行中 / 無 active bridge） | log.debug 忽略 |

B 路徑利用 CLI Bridge 既有的插話機制：`send()` 會 abort 前一個 turn，Claude Code 收到編輯通知後重新回覆。
...
建立已綁定 `messageCreate` + `messageUpdate` + `interactionCreate` handler 的 Client（尚未 `login`）。

> **重要**：不接收 config 參數。每次事件都讀全域 `config`，支援 hot-reload。
...
| `debounceMessageIndex` | `Map<string, {key, idx}>` | messageId → buffer 位置（messageUpdate 替換用） |
